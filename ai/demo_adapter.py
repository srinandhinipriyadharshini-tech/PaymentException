...
class DemoAIAdapter(AIAdapter):
    def extract_claim(self, claim_text: str) -> ExtractedFacts:
        ...
        lower_amounts = [Decimal(value.replace(",", "")) for value in re.findall(r"(?:above|over|more than|greater than)\s+(?:[$£]|gbp\s*)?(\d+(?:,\d{3})?(?:\.\d{1,2})?)\b", lowered, re.I)]
        upper_amounts = [Decimal(value.replace(",", "")) for value in re.findall(r"(?:below|under|less than)\s+(?:[$£]|gbp\s*)?(\d+(?:,\d{3})?(?:\.\d{1,2})?)\b", lowered, re.I)]
        ...
        amount_min = min(amounts + lower_amounts) if amounts or lower_amounts else None
        amount_max = max(amounts + upper_amounts) if amounts or upper_amounts else None
        if lower_amounts and not amounts:
            amount_max = None
        if upper_amounts and not amounts:
            amount_min = None
    ...
    def rank_candidates(self, facts: ExtractedFacts, candidates: list[Payment]) -> list[CandidateMatch]:
        ...
        for payment in candidates:
            score, reasons = 0.2, []
            score = 0.0
            if facts.amount_min is not None and payment.amount >= facts.amount_min and (facts.amount_max is None or payment.amount <= facts.amount_max): score += 0.4; reasons.append("amount")
